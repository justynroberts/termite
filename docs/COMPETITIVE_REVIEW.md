...
| Persistent workspace/tab restoration | Termius/Tabby/Warp support restoration or launch configs | Implemented for SSH tabs and split topology |
| Searchable command history and command blocks | Strongest in Warp | Basic terminal scrollback only |
| Startup commands/login scripts and environment variables | Termius/Tabby | Implemented per host |
...
1. Searchable per-host command history and session logs with redaction.
2. Parameterized runbooks and reusable launch configurations.
3. Optional Mosh and agent forwarding.
4. Import/export of encrypted local workspace backups.
5. Configurable keyboard shortcuts and command palette.
